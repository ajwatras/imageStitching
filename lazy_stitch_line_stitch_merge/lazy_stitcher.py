...
class lazy_stitcher:
    ...
    def stitch(self, main_view_frame, side_view_frames):
        ...
        for i in range(len(side_view_frames)):
            if side_view_has_motion[i]:
                (_, transformed_side_view, _, _, _, _) = self.line_stitch(main_view_frame, side_view_frames[i], i)#sti.applyHomography(main_view_frame, side_view_frames[i], self.homography_list[i])
