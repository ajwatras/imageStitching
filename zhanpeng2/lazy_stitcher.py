...
class lazy_stitcher:
    def __init__(self, main_view_frame, side_view_frames):
        ...
        for i in range(len(side_view_frames)):
            if i == 1:
                (_, _, H, _, _, _) = sti.stitch([side_view_cal_images[0], side_view_cal_images[1]], showMatches=True)
                H = np.dot(self.homography_list[0], H)
                self.homography_list.append(H)
                result1,result2,coord_shift = sti.shiftImage(side_view_cal_images[1],main_view_cal_image,H)
